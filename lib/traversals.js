...
import { isGenerator, isPromise, isFunction } from './general';
...
export async function* traversal(opts) {
  if (!opts) return;
  const {
    handler,
    additionalArgs,
  ...
  if (isFunction(preTraversal)) {
    const ret = preTraversal();
