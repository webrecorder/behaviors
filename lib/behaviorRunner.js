...
export class BehaviorRunner {
  ...
  constructor({ behaviorStepIterator, postStepFN, metadata }) {
     ...
     * system.
     * @type {function(result: {done: boolean, value: *}): {done: boolean, wait: boolean, msg: *, state: *}}
     */
   ...
   * @param {function(results: {done: boolean, value: *}): {done: boolean, wait: boolean, state: *}} [newPostStepFN] - an optional new post step function
   ...
   * @param {function(results: {done: boolean, value: *}): {done: boolean, wait: boolean, msg: string, state: *}} newPostStepFN - The new
   ...
   * @return {Promise<{done: boolean, wait: boolean, msg: string, state: *}>}
   */
  performStep() {
   ...
   * steps
   * @return {AsyncIterableIterator<{done: boolean, wait: boolean, msg: string, state: *}>}
   */
  ...
  async autoRun(options = {}) {
    ...
    let next = await this.step();
    while (!next.done) {
      if (logging) console.log(next.msg);
      if (haveDelay) await delay(delayAmount);
      next = await this.step();
    }
 ...
 * @property {function(results: {done: boolean, value: *}): {done: boolean, wait: boolean, state: *}} [postStepFN] - An optional
