...
export class BehaviorRunner {
   ...
   * @return {Promise<void>}
   */
  async autoRun({ delayAmount, noOutlinks = true, logging = false } = {}) {
   ...
   * @param {number} [delayAmount = 1000] - Optional time in milliseconds
   * the wait after performing an behavior action (step)
   * @return {Promise<void>}
   */
  async autoRunWithDelay({ delayAmount = 1000, collectOutLinks = true } = {}) {
    window.$WBNOOUTLINKS = collectOutLinks;
    let next = await this.step();
    while (!next.done) {
